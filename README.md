...
It loads as `jujutsu-workflow@skills-dir` on the next session. Update with `git -C ~/.claude/skills/jujutsu-workflow pull` and start a new session; remove it by deleting the directory. This route has no `claude plugin update`.
